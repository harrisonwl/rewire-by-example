...
explode w16 = _definition_elided_

crossbar :: Maybe4 -> W 16 -> Maybe4 
crossbar (Maybe4 x10 x20 x30 x40) w16
...
```
Last, but not least, is that every ReWire specification must contain a start declaration. The start symbol must have type ``ReacT Inp Out I ()``.
```haskell
start :: ReT Inp Out I ()
start = devcrossbar
```
